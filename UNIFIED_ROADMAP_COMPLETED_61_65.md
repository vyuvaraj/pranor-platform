...
| CL.G3 | **Fully Functional Per-Branch Preview Environment with Isolated Subdomain** | Pranor Deploy Preview | Auto-provision isolated preview environments for each Git branch or PR, accessible at `<branch>.preview.servcloud.dev`, with automatic Pranor Gate route registration and teardown on PR merge | [x] | OSS |
