...
| PL.G4 | **`servctl` Cluster-Wide Administration CLI** | Platform CLI | Build `servctl` — a unified cluster administration CLI: `servctl cluster status`, `servctl chaos inject --service=pranor-vault --type=latency --duration=60s`, `servctl deploy canary`, `servctl trace query --service=api` | [x] | OSS |
...
| VS.G5 | **Platform Chaos Injection Sidebar Control Panel** | VSCode Chaos | Add a Chaos Control View in the Pranor sidebar to quickly trigger/abort network delay, CPU stress, and disk throttle experiments across cluster nodes | [x] | OSS |
...
> **What is Missing**: 16 specific high-impact differentiators across Pranor Vault (`pranor-vault bench`, `pranor-vault import`, SQL metadata query API, zero-dependency webhooks, `pranor-vault serve-static`), Pranor Gate (upstream LLM dispatch for smart router, `pranor-gate dashboard` TUI, OpenAPI request validation, response diff replay, per-route cost dashboard, `pranor-gate generate-config` learn mode), and Pranor Pulse (embedded web UI at `/ui/`, HTTP/SSE consumer API, cron-scheduled messages, `pranor-pulse benchmark`, SQLite storage backend, plain WebSocket pub/sub).

| # | Item | Component | Description | Status | Tier |
|---|------|-----------|-------------|--------|:---:|
| **Pranor Vault Differentiators** | | | | | |
| ST.D1 | **`pranor-vault bench` Built-in Performance Benchmarking Command** | Pranor Vault CLI | Add built-in CLI benchmark tool `pranor-vault bench --ops 10000 --concurrency 16 --object-size 4KB` measuring PUT/GET/LIST ops/sec, P50, P95, and P99 latencies without external dependencies | [x] | OSS |
| ST.D2 | **`pranor-vault import` One-Command Cloud Migration Mirror** | Pranor Vault Migration | Add built-in S3 migration command `pranor-vault import --from s3://my-aws-bucket` to mirror external AWS S3/GCS buckets into Pranor Vault seamlessly | [x] | OSS |
...
| ST.D5 | **`pranor-vault serve-static` Instant Static Site Hosting** | Pranor Vault Web | Turn any bucket into a static website host (`pranor-vault serve-static --bucket my-site --port 3000`) with proper MIME detection, index.html fallback, and 404 handling | [x] | OSS |
...
| SG.D2 | **`pranor-gate dashboard` Terminal Live Traffic Dashboard (TUI)** | Pranor Gate CLI | Build a terminal-based live htop-style dashboard showing real-time RPS, active connections, circuit breaker state, cache hit rate, and P99 latency per route | [x] | OSS |
...
| SG.D6 | **`pranor-gate generate-config` Zero-Config Learn Mode** | Pranor Gate Config | Run Pranor Gate in learn mode to inspect unrouted traffic and auto-generate `config.json` with discovered routes, rate limits, and circuit breaker thresholds | [x] | OSS |
| **Pranor Pulse Differentiators** | | | | | |
| SQ.D1 | **Standalone Embedded Web Management UI (`/ui/`)** | Pranor Pulse UI | Lightweight browser UI embedded directly in `pranor-pulsed` at `/ui/` showing active topics, consumer lag, DLQ browser with one-click replay, and schema registry | [x] | OSS |
...
| SQ.D4 | **`pranor-pulse benchmark` Built-in Queue Throughput Test** | Pranor Pulse CLI | Add built-in CLI benchmark tool `pranor-pulse benchmark --messages 100000 --producers 8 --consumers 4` measuring msg/sec throughput and latency percentiles | [x] | OSS |
