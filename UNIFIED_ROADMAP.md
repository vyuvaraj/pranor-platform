...
| SL.H4 | **Migration State Tracking & Rollback Engine** | Serv-lang Storage | Track applied migrations in `_serv_migrations` table, prevent duplicate `ALTER TABLE` runs, and support `up {}` / `down {}` migration blocks | [ ] | OSS |
...
| SL.H9 | **`serv.toml` Package Dependency & Resolution Engine** | Serv-lang Package | Add `serv.toml` package declaration manifest and `serv add <pkg>` command with clear resolution order (local -> stdlib -> registry) | [ ] | OSS |
